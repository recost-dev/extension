...
exports.graphqlMatcher = {
    ...
    matchLine(line) {
        ...
        const rawFetchRegex = /fetch\(\s*['"`]([^'"`]*graphql[^'"`]*)['"`][\s\S]{0,500}?JSON\.stringify\(\s*\{[\s\S]{0,400}?query\s*:\s*([\s\S]{1,300}?)(?:,\s*variables\s*:|\})/gi;
        let rawFetchMatch;
