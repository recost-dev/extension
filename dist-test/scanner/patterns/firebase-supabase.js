...
exports.firebaseSupabaseMatcher = {
    ...
    matchLine(line) {
        ...
        while ((supabaseTableMatch = supabaseTableRegex.exec(line)) !== null) {
            ...
            const reg = (0, registry_1.lookupMethod)("supabase", pattern);
            const fbMethod = action === "select" ? "GET" : action === "delete" ? "DELETE" : action === "update" ? "PATCH" : "POST";
            if (!reg)
                console.warn(`[fingerprints] no registry entry for supabase/${pattern}`);
            matches.push({
                kind: "sdk",
                provider: "supabase",
                sdk: "supabase-js",
        ...
        while ((firebaseDocMatch = firebaseDocRegex.exec(line)) !== null) {
            ...
            const reg = (0, registry_1.lookupMethod)("firebase", action);
            const fbMethod = action === "getDoc" || action === "getDocs"
                ? "GET"
                : action === "deleteDoc"
                    ? "DELETE"
                    : action === "updateDoc"
                        ...
                        : "POST";
            if (!reg)
                console.warn(`[fingerprints] no registry entry for firebase/${action}`);
            matches.push({
                kind: "sdk",
                provider: "firebase",
                sdk: "firebase",
