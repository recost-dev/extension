...
function buildOpenAiPath(chain, action) {
    ...
    if (resources.length === 0)
        return null;
    if (!OPENAI_ROOTS.has(resources[0]))
        return null;
    const normalized = resources.map((segment) => (0, utils_1.toSnakeCase)(segment));
    ...
    if (action === "list_files")
        suffix = "/files";
    if (action === "content" || action === "retrieve_content" || action === "download_content")
        suffix = "/content";
    if (action === "cancel" ||
        action === "pause" ||
        action === "resume" ||
    ...
    if (action === "create_and_run" || action === "create_and_run_poll" || action === "create_and_run_stream") {
        if (basePath.endsWith("threads"))
            suffix = "/runs";
    }
    return `/v1/${basePath}${suffix}`;
}
function readBaseUrlMap(line) {
    const baseUrlByClient = new Map();
...
function buildAbsoluteEndpoint(baseUrl, path) {
    if (!baseUrl) {
    ...
    return { endpoint, host: (0, utils_1.parseHost)(endpoint) };
}
exports.openAiCompatibleMatcher = {
    ...
    matchLine(line) {
        ...
        while ((match = OPENAI_ACTION_REGEX.exec(line)) !== null) {
            ...
            if (action === "unwrap" || action === "verify_signature")
                continue;
