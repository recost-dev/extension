...
export const vertexMatcher: LineMatcher = {
  name: "provider-vertex",
  matchLine(line: string): ApiCallMatch[] {
    const matches: ApiCallMatch[] = [];

    const sdkRegex =
      /\b(?:vertex|vertexAi|vertexAI|client|generativeModel)\.(generateContent|streamGenerateContent|generate_content|stream_generate_content|embedContent|embed_content)\s*\(/gi;
    ...
    const restRegex =
      /https?:\/\/[a-z0-9-]+-aiplatform\.googleapis\.com\/v1\/projects\/[^\s'"`]+\/locations\/[^\s'"`]+\/(?:publishers\/google\/models\/[^\s'"`]+:(?:generateContent|streamGenerateContent|predict)|endpoints\/[^\s'"`]+:(?:generateContent|streamGenerateContent|predict))/gi;
