...
debugLog("[BUNDLE] wasmDir:", wasmDir, "| exists:", fs.existsSync(wasmDir));
...
async function ensureInitialized(): Promise<void> {
  ...
  _initPromise = (async () => {
    ...
    debugLog("[BUNDLE] Parser.init() — runtime WASM:", runtimeWasm, "| exists:", fs.existsSync(runtimeWasm));
    await _Parser!.init({
    ...
    _initialized = true;
    debugLog("[BUNDLE] Parser.init() — success");
  })();
...
async function loadGrammar(langName: string): Promise<import("web-tree-sitter").Language> {
  ...
  debugLog("[BUNDLE] loadGrammar:", langName, "—", wasmPath, "| exists:", fs.existsSync(wasmPath));
